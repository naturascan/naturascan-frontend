...
class PatListWidget extends StatefulWidget {
  final List<UserModel>? selectedUser;

  const PatListWidget({super.key, this.selectedUser});

  @override
  State<PatListWidget> createState() => _PatListWidgetState();
}

class _PatListWidgetState extends State<PatListWidget> {
  @override
  void initState() {
    getUserList();
    super.initState();
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        CustomInputField(
          readOnly: true,
          hint: "Ajouter un patrouilleur",
          suffix: IconButton(
              onPressed: () {
                Get.to(() => const AddUserScreen(type: 3));
              },
              icon: const Icon(
          ...
          onTap: () {
            Get.bottomSheet(
                    SelectAndAddUserWidget(selectedUser: widget.selectedUser, type: 3,))
                .then((value) => setState(() {}));
          },
        ),
