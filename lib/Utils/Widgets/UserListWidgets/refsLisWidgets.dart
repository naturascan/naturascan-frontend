...
class RefsListWidget extends StatefulWidget {
  final List<UserModel>? selectedUser;

  const RefsListWidget({super.key, this.selectedUser});

  @override
  State<RefsListWidget> createState() => _RefsListWidgetState();
}

class _RefsListWidgetState extends State<RefsListWidget> {
  @override
  void initState() {
    getUserList();
    super.initState();
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        CustomInputField(
          readOnly: true,
          hint: "Ajouter un référent secteur",
          suffix: IconButton(
              onPressed: () {
                Get.to(() => const AddUserScreen(type: 2));
              },
              icon: const Icon(
          ...
          onTap: () {
            Get.bottomSheet(
                    SelectAndAddUserWidget(selectedUser: widget.selectedUser, type: 2,))
                .then((value) => setState(() {}));
          },
        ),
